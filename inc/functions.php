...
<ul>
    ...
    <?php if(defined( 'VBEANERRORMAILER_PLUGIN_VERSION' )){ 
        echo('<a href="'.esc_url($adminurl.'admin.php?page=vanilla-bean-errormailer/vanillabean-errormailer.php').'">Error Mailer</a> (installed)');
    }else{
        echo('<a href="'.esc_url($adminurl.'plugin-install.php?tab=search&type=term&s=vanilla+bean+error+mailer').'">Error Mailer</a>');
    ...
    <?php if(defined( 'VBEANTHEMELOGIN_PLUGIN_VERSION' )){ 
        echo('<a href="'.esc_url($adminurl).'admin.php?page=vanilla-bean-themelogin/vanilla-bean-themelogin.php">Theme Login</a> (installed)');
        }else {
        echo('<a href="'.esc_url($adminurl).'plugin-install.php?tab=search&type=term&s=vanilla+bean+theme+login">Theme Login</a>');
     }?>
        <div class="small-text">Themify your login and password recovery pages.</div>
    </li>
   <li>
    <?php if(defined( 'VBEANFAVICON_PLUGIN_VERSION' )){ 
       echo('<a href="'.esc_url($adminurl).'admin.php?page=vanilla-bean-icon-setter/vanilla-bean-icon-setter.php">Icon Setter</a> (installed)');
    }else {
        echo('<a href="'.esc_url($adminurl).'plugin-install.php?tab=search&type=term&s=vanilla+bean+icon+setter">Icon Setter</a>');
     }?>
...
if(!function_exists('\VanillaBeans\LiveSettings')){
	...
	global $mmtabs, $mmtab, $paged, $type, $term;
	$mmtabs = array();
	$mmtab = "search";
	$per_page = 10;
	$args = array
	...
	$arges = apply_filters( "install_plugins_table_api_args_$mmtab", $args );
										...
										if (current_user_can( "install_plugins") || current_user_can("update_plugins"))
										{
											...
											switch ($status["status"])
											{
												...
												case "newer_installed":
													$action_links[] = '<span class="button button-disabled '.$plugin["slug"] .'" title="' . esc_attr__( "This plugin is already installed and is up to date" ) . ' ">' . _x( 'Installed', 'plugin' ) . '</span>';
												break;
											}
										}
										?>
											...
											<div class="plugin-div-top plugin-div-settings-top">
												<div class="plugin-div-inner-content">
													<a href="<?php echo esc_url( $details_link ); ?>" class="thickbox plugin-icon plugin-icon-custom">
														<img class="custom_icon" style="width:128px;height:128px;" src="<?php echo esc_attr( $plugin_icon_url ) ?>" />
													</a>
